...
tg <- expand.grid(shrinkage = 0.01, 
                  interaction.depth = seq(10, 15, by = 1),
                  n.minobsinnode = seq(1, 10, by = 1),
                  n.trees = 1000)

# Verbose is a parameter sent to the underlying modeling function
gbm_model <- train(Consumption ~ ., data = training, 
                    method = "gbm", trControl = control, tuneGrid =tg, verbose = FALSE)
...
predictions = as_tibble(predict(gbm_model, newdata = test.features))
...
ggplot(my_data,aes(Predicted, Observed)) + geom_point(color = "darkred", alpha = 0.5) + 
    geom_smooth(method=lm) + ggtitle("GBM Regression: Prediction vs Test Data") +
...
# We use expand grid to create a table of all possible combinations
tg <- expand.grid(shrinkage = 0.01, 
                  interaction.depth = seq(5, 10, by = 1),
                  n.minobsinnode = seq(1, 10, by = 1),
                  n.trees = 1000)
...
# We use expand grid to create a table of all possible combinations
tg <- expand.grid(shrinkage = 0.01, 
                  interaction.depth = seq(5, 15, by = 1),
                  n.minobsinnode = seq(1, 10, by = 1),
                  n.trees = 1000)
...
```{r Model Stats}
df_gbm <- data.frame (zone = c("Lower", "Upper", "Intermediate", "Foothill", "Mountain", "34 Hydro", "59 Hydro", "101 Hydro", "149 Hydro", "Canal 1", "Canal 2", "Canal 3"), mae, rmse, r2)
df_gbm
```