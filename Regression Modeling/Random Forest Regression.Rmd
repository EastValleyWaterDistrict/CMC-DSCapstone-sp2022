...
#create tunegrid with 10 values from 5:15 for mtry to tuning model. Our train function will change number of entry variable at each split according to tunegrid. 
tunegrid <- expand.grid(.mtry = (5:15))  

rf_model <- train(Consumption ~ ., 
                       data = training,
                       method = 'rf',
                       tuneGrid = tunegrid)

arrange(rf_model$results, RMSE) %>% head

test.features = subset(testing, select=-c(Consumption))
test.target = subset(testing, select=Consumption)[,1]

predictions = as_tibble(predict(rf_model, newdata = test.features))
...
ggplot(my_data,aes(Predicted, Observed)) + geom_point(color = "darkred", alpha = 0.5) + 
    geom_smooth(method=lm) + ggtitle("Random Forest Regression: Prediction vs Test Data") +
...
#create tunegrid with 10 values from 1:10 for mtry to tuning model. Our train function will change number of entry variable at each split according to tunegrid. 
...
```{r Model Stats}
df_rf <- data.frame (zone = c("Lower", "Upper", "Intermediate", "Foothill", "Mountain", "34 Hydro", "59 Hydro", "101 Hydro", "149 Hydro", "Canal 1", "Canal 2", "Canal 3"), mae, rmse, r2)
df_rf
```