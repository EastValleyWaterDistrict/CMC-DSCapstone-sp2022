...
arrange(svm_model$results, RMSE) %>% head

test.features = subset(testing, select=-c(Consumption))
test.target = subset(testing, select=Consumption)[,1]

predictions = as_tibble(predict(svm_model, newdata = test.features))
...
ggplot(my_data,aes(Predicted, Observed)) + geom_point(color = "darkred", alpha = 0.5) + 
    geom_smooth(method=lm) + ggtitle("SVM Regression: Prediction vs Test Data") +
...
```{r Model Stats}
df_svm <- data.frame (zone = c("Lower", "Upper", "Intermediate", "Foothill", "Mountain", "34 Hydro", "59 Hydro", "101 Hydro", "149 Hydro", "Canal 1", "Canal 2", "Canal 3"), mae, rmse, r2)
df_svm
```