...
train.features = data.matrix(training %>% dplyr::select(-Consumption))
train.target = training$Consumption
test.features = data.matrix(testing %>% dplyr::select(-Consumption))
test.target = testing$Consumption
...
#Define grid space to search for the best hyperparameters
xgbGrid <- expand.grid(nrounds = seq(150, 250, by = 50),
                       max_depth = seq(1, 4, by = 1),
                       colsample_bytree = seq(0.1, 1.0, by = 0.1),
...
predictions = as_tibble(predict(xgb_model, newdata = test.features))
...
ggplot(my_data,aes(Predicted, Observed)) + geom_point(color = "darkred", alpha = 0.5) + 
    geom_smooth(method=lm) + ggtitle("XGBoost Regression: Prediction vs Test Data") +
...
#Define grid space to search for the best hyperparameters
xgbGrid <- expand.grid(nrounds = 120,
                       max_depth = seq(1, 4, by = 1),
                       colsample_bytree = seq(0.1, 1.0, by = 0.1),
                       ## The values below are default values in the sklearn-api 
                       eta = 0.1,
...
#Define grid space to search for the best hyperparameters
xgbGrid <- expand.grid(nrounds = 60,
                       max_depth = seq(1, 4, by = 1),
                       colsample_bytree = seq(0.1, 1.0, by = 0.1),
                       ## The values below are default values in the sklearn-api 
                       eta = 0.1,
...
#Define grid space to search for the best hyperparameters
xgbGrid <- expand.grid(nrounds = 240,
                       max_depth = seq(1, 4, by = 1),
                       colsample_bytree = seq(0.1, 1.0, by = 0.1),
                       ## The values below are default values in the sklearn-api 
                       eta = 0.1,
...
```{r Model Stats}
df_xgb <- data.frame (zone = c("Lower", "Upper", "Intermediate", "Foothill", "Mountain", "34 Hydro", "59 Hydro", "101 Hydro", "149 Hydro", "Canal 1", "Canal 2", "Canal 3"), mae, rmse, r2)
df_xgb
```